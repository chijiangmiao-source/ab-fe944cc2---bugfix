...
- **可复算记录**：每一步选择都能仅凭输入与响应公开的证据复算。
  - 逐层选择规则公开为比较键 `(本层修正代价, 规范优先标记, 通道标识)`
    （规范树通道标记 0，其余 1，逐分量取最小）；
  - 每一层公开各非根（超）点的**全部候选入边及其比较键**与选中结果；
  - 每次有向环收缩（环节点/环边、超点、入边代价 `w − w*(v)` 修正、丢弃的环内边）；
  - 每次展开替换（进入通道、进入点、被替换环边、保留环边）；
  - 规范裁决序列：按标识升序逐条公开强制入选的最小总代价与接受与否。
  - `replay_record()` 只依据输入与公开记录即可逐步重算出同一棵树并做结构校验。
...
`record.key_scheme` / `record.levels`（含每层全部候选及比较键）/
`record.expansions` / `record.canonical_decisions`；无解返回
`status: "unsolvable"` 与 `unreachable`；输入错误返回 HTTP 422、
`status: "invalid"` 与 `errors`。

## 算法要点

1. 每个非根点按公开比较键 `(本层修正代价, 规范优先标记, 通道标识)` 选最小入边；
   若不入环，即为最优树。候选与键全部随记录公开。
...
5. 规范解：先求最优代价 C*，再按标识升序逐条试探强制入选，
   仅当强制后最优代价仍为 C* 时接受（每一步裁决及其代价依据随
   `record.canonical_decisions` 公开）；最后以 `(代价, 规范优先标记, 标识)`
   为比较键重跑 Edmonds，使产出的记录恰好对应规范树。
